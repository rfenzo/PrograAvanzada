...
class Pizzeria:
    ...
    def sale_old_client(self, name):
        ...
        if client.cupons >= 10:
            ...
            client.cupons -= 10
        print('{} ha comprado una pizza. '.format(client.name) +
              'Número de cupones acomulados {}'.format(client.cupons))
    ...
    def find_client(self, name):
        ...
        for file_name in listdir(self.db_directory):
            if file_name == name+'.json':
                ...
                with open(self.db_directory +
                          '/'+file_name, 'r') as file_client:
                    cliente = load(
                        file_client, object_hook=lambda x: client_decoder(x))
                return cliente

    def sale_new_client(self, name, phone, address):
        # se crea un nuevo cliente
        client = Client(name, phone, address, 0)
        print("Venta a nuevo cliente {}, phone {}, address {}".format(
    ...
    def update_client(self, client):
        ...
        with open(self.db_directory +
                  '/'+client.name+'.json', 'w') as client_file:
